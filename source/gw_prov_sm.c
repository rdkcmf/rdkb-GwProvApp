...
static void *GWP_lxcserver_threadfunc(void *data)
{
    ...
    if ((fd_ser = socket(AF_INET, SOCK_STREAM, 0)) == 0)
    {
        perror("socket failed");
        exit(EXIT_FAILURE);
    }

    if (setsockopt(fd_ser, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
    ...
    if (bind(fd_ser, (struct sockaddr *)&address,
                                 sizeof(address))<0)
    {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }
    if (listen(fd_ser, 3) < 0)
    {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    ...
    while(1)
    {
        if ((new_socket = accept(fd_ser, (struct sockaddr *)&address,
                       (socklen_t*)&addrlen))<0)
        {
            perror("accept");
            exit(EXIT_FAILURE);
